...
export function createTypescriptTemplate(workbenchFile: ApolloWorkbenchFile) {
    ...
    for (var serviceName in workbenchFile.schemas) {
        ...
        archive.append(generateTsConfig(), { name: `${serviceFolder}/tsconfig.json` });
        archive.append(generateTsFederatedServerPackageJson(serviceName), { name: `${serviceFolder}/package.json` });
        archive.append(workbenchFile.schemas[serviceName].sdl, { name: `${serviceSrcFolder}/schema.graphql` });
        archive.append(generateTsFederatedServerTemplate(port, serviceName), { name: `${serviceSrcFolder}/index.ts` });
        archive.append(generateTsVsCodeLaunch(serviceName), { name: `${serviceVsCodeFolder}/launch.json` });
