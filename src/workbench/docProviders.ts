...
export class ApolloStudioOperationsProvider implements TextDocumentContentProvider {
    static scheme: string = 'apollo-studio-operations';
    static Uri(operationName: string, document: string): Uri {
        return Uri.parse(`${ApolloStudioOperationsProvider.scheme}:${operationName}.graphql?${document}`);
    }
    provideTextDocumentContent(uri: Uri): string {
